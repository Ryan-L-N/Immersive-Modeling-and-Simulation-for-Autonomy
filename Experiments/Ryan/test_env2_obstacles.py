...
# =============================================================================
# CREATE EVENLY DISTRIBUTED OBSTACLES
# =============================================================================

def create_obstacles():
    """Create evenly distributed obstacles (boxes and cylinders).

    Obstacles are placed in a deterministic grid pattern between walls,
    with generous spacing and avoiding start/goal zones.
    """
    # Convert lbs to kg for physics
    WEIGHT_10_LBS = 4.54  # kg
    WEIGHT_100_LBS = 45.4  # kg
    MAX_HEIGHT_FT = 2.0
    MAX_HEIGHT_M = MAX_HEIGHT_FT * 0.3048  # 0.6096 meters

    # Create a grid pattern with generous spacing
    # Arena is 100m x 50m, we'll place obstacles with ~8-10m spacing
    x_spacing = 10.0
    y_spacing = 8.0

    # Grid starts at -40 to +40 in X (avoiding start/goal zones at -45 and +45)
    # Grid spans -20 to +20 in Y
    x_positions = np.arange(-40, 41, x_spacing)
    y_positions = np.arange(-20, 21, y_spacing)

    # Exclude zones near start (-45 to -35) and goal (+35 to +45)
    x_positions = [x for x in x_positions if not (-35 <= x <= 35 and abs(x - config.START_X) < 5 or abs(x - config.GOAL_X) < 5)]

    obstacle_count = 0

    # Deterministic pattern for obstacle types and weights
    # Alternate between boxes and cylinders, and between light and heavy
    for i, x in enumerate(x_positions):
        for j, y in enumerate(y_positions):
            # Skip if in start or goal zones
            if (-50 <= x <= -38) or (38 <= x <= 50):
                continue

            # Avoid robot start positions
            too_close_to_start = False
            for start_y in config.START_Y_POSITIONS:
                if abs(y - start_y) < 3.0 and abs(x - config.START_X) < 8.0:
                    too_close_to_start = True
                    break
            if too_close_to_start:
                continue

            # Deterministic obstacle type based on position
            is_box = (i + j) % 2 == 0

            # Deterministic weight based on position
            mass = WEIGHT_10_LBS if (i * 3 + j * 2) % 3 == 0 else WEIGHT_100_LBS

            # Deterministic height based on position (vary between 0.3 and max)
            height_ratio = 0.5 + 0.5 * ((i + j) % 3) / 2.0
            height = 0.3 + height_ratio * (MAX_HEIGHT_M - 0.3)

            if is_box:
                # Box dimensions (deterministic based on position)
                width = 0.4 + 0.3 * ((i % 3) / 2.0)
                depth = 0.4 + 0.3 * ((j % 3) / 2.0)

                # Create box
                obstacle = UsdGeom.Cube.Define(stage, f"/World/Obstacles/Box_{obstacle_count}")
                xform = UsdGeom.Xformable(obstacle.GetPrim())
                xform.AddTranslateOp().Set(Gf.Vec3d(x, y, height/2))
                xform.AddScaleOp().Set(Gf.Vec3f(width, depth, height))

                # Color based on weight (lighter for 10lbs, darker for 100lbs)
                if mass == WEIGHT_10_LBS:
                    color = Gf.Vec3f(0.6, 0.5, 0.4)  # Light brown
                else:
                    color = Gf.Vec3f(0.4, 0.35, 0.3)  # Dark brown
                obstacle.GetDisplayColorAttr().Set([color])

            else:
                # Cylinder
                radius = 0.25 + 0.15 * ((i + j) % 3) / 2.0

                # Create cylinder
                obstacle = UsdGeom.Cylinder.Define(stage, f"/World/Obstacles/Cylinder_{obstacle_count}")
                xform = UsdGeom.Xformable(obstacle.GetPrim())
                xform.AddTranslateOp().Set(Gf.Vec3d(x, y, height/2))
                xform.AddScaleOp().Set(Gf.Vec3f(radius, radius, height/2))

                # Color based on weight
                if mass == WEIGHT_10_LBS:
                    color = Gf.Vec3f(0.55, 0.55, 0.55)  # Light gray
                else:
                    color = Gf.Vec3f(0.35, 0.35, 0.35)  # Dark gray
                obstacle.GetDisplayColorAttr().Set([color])

            # Add physics
            prim = obstacle.GetPrim()
            ...
            material_api.CreateRestitutionAttr().Set(0.1)

            obstacle_count += 1

    light_count = sum(1 for i in range(obstacle_count) if (i % 3) == 0)
    heavy_count = obstacle_count - light_count

    print(f"Created {obstacle_count} evenly distributed obstacles:")
    print(f"  Light (10 lbs / {WEIGHT_10_LBS} kg): ~{light_count} obstacles")
    print(f"  Heavy (100 lbs / {WEIGHT_100_LBS} kg): ~{heavy_count} obstacles")
    print(f"  Max height: {MAX_HEIGHT_FT} feet ({MAX_HEIGHT_M:.2f} meters)")
    print(f"  Grid spacing: {x_spacing}m x {y_spacing}m")
